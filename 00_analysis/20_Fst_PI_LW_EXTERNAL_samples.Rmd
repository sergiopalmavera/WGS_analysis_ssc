---
title: "Fst analysis - External samples - PI vs LW"
output: 
  html_document:
    toc: 5
---
...
fst_win_pi_lw <- vroom(here("13_Fst/output/fst_win_external_PT_vs_LW.windowed.weir.fst"))

fst_win_pi_lw$MEAN_FST %>% range()

fst_win_pi_lw$MEAN_FST %>% summary()

fst_win_pi_lw %>% dim() #89346     6
...
Genomic mean differentiation is 0.11 (among SOSFERT_samples: 0.15). 99th quantile windows have an Fst >= 0.44 (SOSFERT_samples: 0.59), but still this is not very extreme (zFst = 3.31 (SOSFERT_samples: 3.26)). None of the windows have an Fst = 1
...
png(here("00_analysis/figures/manhattan_genomewide_fst_pi_lw_external_outliers.png"), res = 300, width = 4000, height = 2000, units = "px"); print(p_man_fst_autosomes); dev.off()
png(here("00_analysis/figures/manhattan_genomewide_z_fst_pi_lw_external_outliers.png"), res = 300, width = 4000, height = 2000, units = "px"); print(p_man_z_fst_autosomes); dev.off()
```

```{r display_manhattan_plots}

knitr::include_graphics(c(here("00_analysis/figures/manhattan_genomewide_fst_pi_lw_external_outliers.png"),
                        here("00_analysis/figures/manhattan_genomewide_z_fst_pi_lw_external_outliers.png")))
...
genes_outliers %>% saveRDS(here("00_analysis/data/genes_outliers_Fst_PI_LW_external_samples.rds"))
```
...
run_WebGestaltR(
  ...
  outputDirectory=here("00_analysis/data/WebGestaltR_fst_PI_LW_EXTERNAL_samples")
)
...
ssc_outlier_genes_to_hsa$ensembl_gene_id %>% unique() %>% length() #163

ssc_outlier_genes_to_hsa %>% filter(hsapiens_homolog_ensembl_gene != "") %>% .$hsapiens_homolog_ensembl_gene %>% unique() %>% length() #147
...
inner_join(ssc_outlier_genes_to_hsa, ssc_gene_set_with_descr, by = c("ensembl_gene_id"="gene_id")) %>% 
  as_tibble() %>% 
  unique() %>% 
  write.csv(here("00_analysis/data/WebGestaltR_fst_PI_LW_EXTERNAL_samples/Project_GOBP_hsa_ortho/ssc_to_hsa_orthologs_and_gene_information.csv"))
...
This time, significant results were found for immunological processes and protein catabolism.

```{r import_significant_results}

webgestalt_ortho_res <- here("00_analysis/data/WebGestaltR_fst_PI_LW_EXTERNAL_samples/Project_GOBP_hsa_ortho/geneontology_Biological_Process_sig_results.csv") %>% 
  vroom()

webgestalt_ortho_res %>% 
