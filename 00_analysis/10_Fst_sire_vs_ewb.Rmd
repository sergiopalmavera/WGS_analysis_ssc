...
fst_win_sire_ewb$MEAN_FST %>% range()

fst_win_sire_ewb$MEAN_FST %>% summary()

fst_win_sire_ewb %>% dim() #89876     6

sum(fst_win_sire_ewb$MEAN_FST > 0.5) 
...
data_cum <- fst_win_sire_ewb_prep %>% 
...
fst_win_sire_sire_prep_manhattan <- fst_win_sire_ewb_prep %>% 
  mutate(center = (BIN_START + (BIN_END-BIN_START)/2 )) %>% 
  inner_join(data_cum, by = "CHROM") %>% 
  mutate(bp_cum = center + bp_add)


axis_set <- fst_win_sire_sire_prep_manhattan %>% 
  group_by(CHROM) %>% 
  summarise(center = mean(bp_cum))

p_man_fst_autosomes <- fst_win_sire_sire_prep_manhattan %>% 
  ...
  geom_hline(yintercept = quantile(fst_win_sire_ewb_prep$MEAN_FST, c(0.99, 0.999)), linetype = "dashed", color = "red", size = 1, alpha = 0.7) +
  theme(
...
p_man_z_fst_autosomes <- fst_win_sire_sire_prep_manhattan %>% 
  ...
  labs(x = NULL) +
  theme_minimal(base_size = 12) +
  geom_hline(yintercept = quantile(fst_win_sire_ewb_prep$zFst, c(0.99, 0.999)), linetype = "dashed", color = "red", size = 1, alpha = 0.7) +
  theme(
    legend.position = "none",
    panel.border = element_blank(),
    panel.grid.major.x = element_blank(),
...
png(here("00_analysis/figures/manhattan_genomewide_fst_sire_ewb_outliers.png"), res = 300, width = 4000, height = 2000, units = "px"); print(p_man_fst_autosomes); dev.off()
png(here("00_analysis/figures/manhattan_genomewide_z_fst_sire_ewb_outliers.png"), res = 300, width = 4000, height = 2000, units = "px"); print(p_man_z_fst_autosomes); dev.off()
```

```{r display_manhattan_plots}

knitr::include_graphics(c(here("00_analysis/figures/manhattan_genomewide_fst_sire_ewb_outliers.png"),
                        here("00_analysis/figures/manhattan_genomewide_z_fst_sire_ewb_outliers.png")))
...
cutoffs <- fst_win_sire_ewb_prep$zFst %>% quantile(c(0.99, 0.999))

outliers_1pct <- fst_win_sire_ewb_prep %>% 
  filter(zFst > cutoffs[1]) %>% 
  makeGRangesFromDataFrame(keep.extra.columns = TRUE) %>% 
  # merge adjacent outliers
  reduce(min.gapwidth = 1)

outliers_0.1pct <- fst_win_sire_ewb_prep %>% 
...
#genes_outliers %>% as_tibble() %>% write.csv(here("00_analysis/data/genes_outliers_fst_sire_ewb.csv"))
...
There were no siginificant results.
...
run_WebGestaltR(
  ...
  outputDirectory=here("00_analysis/data/WebGestaltR_fst_sire_ewb")
)
