---
title: "Fst dam vs sire - outlier analysis"
...
p_man_fst_autosomes <- fst_win_dam_sire_prep_manhattan %>% 
  ...
  geom_hline(yintercept = quantile(fst_win_dam_sire_prep$MEAN_FST, c(0.99, 0.999)), linetype = "dashed", color = "red", size = 1, alpha = 0.7) +
  theme(
...
p_man_z_fst_autosomes <- fst_win_dam_sire_prep_manhattan %>% 
  ...
  labs(x = NULL) +
  theme_minimal(base_size = 12) +
  geom_hline(yintercept = quantile(fst_win_dam_sire_prep$zFst, c(0.99, 0.999)), linetype = "dashed", color = "red", size = 1, alpha = 0.7) +
  theme(
    legend.position = "none",
    panel.border = element_blank(),
    panel.grid.major.x = element_blank(),
...
png(here("00_analysis/figures/manhattan_genomewide_fst_outliers.png"), res = 300, width = 4000, height = 2000, units = "px"); print(p_man_fst_autosomes); dev.off()
png(here("00_analysis/figures/manhattan_genomewide_z_fst_outliers.png"), res = 300, width = 4000, height = 2000, units = "px"); print(p_man_z_fst_autosomes); dev.off()
```

```{r display_manhattan_plots}

knitr::include_graphics(c(here("00_analysis/figures/manhattan_genomewide_fst_outliers.png"),
                        here("00_analysis/figures/manhattan_genomewide_z_fst_outliers.png")))
...
genes_outliers %>% as_tibble() %>% write.csv(here("00_analysis/data/genes_outliers_fst.csv"))
...
There were no significant results (FDR < 0.1) for any of the gene lists.
...
run_WebGestaltR(
  ...
  projectName="WebGestaltR_fst",
  enrichDatabase="geneontology_Biological_Process",
  outputDirectory=here("00_analysis/data/webgestalt_fst/GOBP")
)
...
run_WebGestaltR(
  ...
  outputDirectory=here("00_analysis/data/webgestalt_fst/KEGG")
)
